...
# Define download directory (configurable)
$downloadPath = "C:\Temp\AppInstallers"
$downloadPath = Read-Host "Enter download directory (default: C:\Temp\AppInstallers):" -DefaultValue $downloadPath

# Create download directory only if needed
if (!(Test-Path $downloadPath)) {
  New-Item -ItemType Directory -Path $downloadPath -ErrorAction SilentlyContinue
}

# Function to download and install with silent execution
function Install-Silent {
  ...
  Write-Progress -Activity "Installing" -Status "$appName" -PercentComplete 0

  if ($packageName -ne "") {  # Try Chocolatey first
    Write-Host "  Attempting installation with Chocolatey..."
    choco install $packageName -y  # -y flag for silent installation
    if (!$?) {
      Write-Warning "  Chocolatey installation failed. Falling back to download..."
    } else {
      Write-Progress -Activity "Installing" -Status "$appName" -PercentComplete 100
      return  # Exit function if Chocolatey succeeds
    }
  }

  if ($downloadUrl -ne "") {  # Download installer only if Chocolatey fails
    Write-Host "  Chocolatey installation failed. Downloading installer..."
    try {
      Invoke-WebRequest -Uri $downloadUrl -OutFile "$downloadPath/$installerFilename"
    } catch {
      Write-Error "  Failed to download $appName installer: ($_.Exception.Message)"
      return
    }

    Write-Host "  Installing $appName silently..."
    Start-Process -FilePath "$downloadPath/$installerFilename" -ArgumentList $installerArgs -Wait -WindowStyle Hidden

    Remove-Item "$downloadPath/$installerFilename" -ErrorAction SilentlyContinue
  }

  Write-Progress -Activity "Installing" -Status "$appName" -PercentComplete 100
}

# Assuming the winget installation happens from the GitHub script (not included here)
# ... (Replace this with the winget installation script from GitHub)

# Check for and update winget (assuming the GitHub script handles this)
# ... (Remove this section as it's likely handled by the GitHub script)

# Install Chocolatey if not found
if (!(Test-Path -Path "$env:ProgramFiles\chocolatey\chocolatey.exe")) {
  Write-Host "Installing Chocolatey..."
  Invoke-WebRequest -Uri https://community.chocolatey.org/api/v2/install.ps1 -OutFile chocolateyInstall.ps1
  . .\chocolateyInstall.ps1
  Remove-Item chocolateyInstall.ps1
}

# Install applications with prioritization
$wingetApps = @("Mozilla.Firefox", "Notepad++.Notepad++", "7zip.7zip", "Speccy", "RevoUninstaller.RevoUninstaller", "Malwarebytes.Malwarebytes", "VirusTotal.VirusTotalUploader", "Microsoft.WindowsTerminal")
$chocoApps = @("python3" -Name "python")  # Example using Chocolatey name convention

$installedApps = @()

foreach ($appName in $wingetApps) {
  # Try winget installation with error handling (silent)
  try {
    winget install --id $appName --exact -Quiet
    if ($?) {
      $installedApps += $appName
      Write-Progress -Activity "Installing" -Status "$appName (winget)" -PercentComplete 100
    }
  } catch {
    Write-Warning "winget installation of $appName failed: ($_.Exception.Message)"
  }
}

foreach ($app in $chocoApps) {
  Install-Silent -appName ($app.Name) -packageName $app.Name -Silent  # Explicitly set Silent flag
  if ($?) {
    $installedApps += $app.Name
  }
}

# Summarize installed applications
if ($installedApps.Count -gt 0) {
  Write-Host "Successfully installed applications:"
  Write-Host ($installedApps -join "`n * ")
} else {
  Write-Host "No applications were installed."
}

Write-Host "All other installations completed (if applicable
